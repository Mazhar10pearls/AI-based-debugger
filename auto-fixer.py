...
{signal}
```
## File contents (base your fix on these):
{context}"""


def build_prompt(signal, context, stacks):
    user = USER_PROMPT.format(stacks=", ".join(sorted(stacks)) or "unknown",
                              signal=signal, context=context)
    full = SYSTEM_PROMPT + "\n\n" + user
    cap = 9000
    if len(full) > cap:
        allowed = cap - len(SYSTEM_PROMPT) - 300
        context = context[:max(allowed, 1000)] + "\n...(trimmed)"
        user = USER_PROMPT.format(stacks=", ".join(sorted(stacks)) or "unknown",
                                  signal=signal, context=context)
    return user
...
def main():
    ...
    if not fixes:
        ...
        if token and repo:
            open_issue(token, repo, f"AI produced no fixes. Root cause: {root_cause}", run_url)
        sys.exit(3)
