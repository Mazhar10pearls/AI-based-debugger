...
No hardcoded file paths. The system:
  ...
  2. DISCOVER  — Extract filenames from the log, read the relevant files,
                 build a repository tree
  3. ANALYSE   — Ask Ollama: the AI does ALL the reasoning — understands the
                 failure, determines root cause, decides whether the files it
                 was shown are sufficient, chooses what to change, generates
                 the fixed code, and explains its root cause. Returns structured
                 JSON. If it says the context isn't enough, it names the exact
                 files it needs and gets exactly one more round with them added.
...
Tuned for a CPU-only 3B model (qwen2.5-coder:3b) on an 8GB host:
  ...
  - If the initial file selection genuinely isn't enough, the model can say so
    (`context_sufficient: false` + `requested_files`) instead of guessing blind
    — it gets exactly one more round with those files read and added.

There is no deterministic "prescan" layer and no build-and-verify retry loop:
every diagnosis and every fix decision is made by the model in a single
(occasionally two-round) pass, matching a straight-line pipeline rather than a
...
# ── Extra context round ───────────────────────────────────────────────────────
# When the model says the initial file selection is insufficient, it names the
# repo paths it needs and gets exactly ONE more read-and-ask round with those
# files appended — never an open-ended loop.
MAX_REQUESTED_FILES  = 5
EXTRA_CONTEXT_BUDGET = 6000  # chars, additive on top of MAX_TOTAL_CONTEXT
...
#
# context_sufficient / requested_files let the model say the files it was shown
# aren't enough to safely diagnose or fix the failure, instead of guessing.
FIX_SCHEMA = {
    "type": "object",
    "properties": {
...
def discover_context(error_signal: str,
    ...
    """
    Extract-filenames-from-log + read-relevant-files + build-repository-tree, in
    one pass: force-include whatever the log explicitly named, then fill the
    remaining slots with score-ranked candidates. No semantic steering of WHAT
    is wrong is done here — that reasoning belongs entirely to the model (see
    STAGE 3), which can ask for more files via context_sufficient/requested_files
    if this initial selection isn't enough.
    """
    ...
    # ── 1. FORCE-INCLUDE files explicitly named in the log ───────────────────
...
def read_extra_files(requested: list[str],
                     already_included: list[str],
                     budget: int = EXTRA_CONTEXT_BUDGET,
                     limit: int = MAX_REQUESTED_FILES) -> tuple[str, list[str]]:
    """
    Resolve and read files the AI explicitly asked for because it judged the
    initial context insufficient (FIX_SCHEMA.requested_files). This is the
    model DECIDING what it needs, not a heuristic guessing on its behalf — we
    just resolve the paths (exact match, or basename match against the repo)
    and read them. Skips anything already shown, blocked, or excluded.
    """
    ...
    for raw in requested[:limit]:
        rel = _relstrip(raw.strip().replace("\\", "/"))
        candidates = [rel] if Path(rel).is_file() else name_to_paths.get(Path(rel).name, [])
        for cand in candidates:
            if cand in already_included or cand in added:
                continue
            if _basename_blocked(Path(cand).name):
                continue
            if any(re.search(pp, cand) for pp in CONTEXT_EXCLUDE_PATTERNS):
                continue
            p = Path(cand)
            if not (p.is_file() and _is_text_file(p)):
                continue
            content = _read_whole_forced(p) or _read_clamped(p)
            size_tag = " (LARGE — use edits)" if len(content) >= EDIT_MODE_THRESHOLD else ""
            block = f"### {cand}{size_tag}\n```\n{content}\n```"
            if total + len(block) > budget and added:
                continue
            parts.append(block)
            added.append(cand)
            total += len(block)
            print(f"[DISCOVER] AI requested extra file → {cand} ({len(content)} chars)")
            break
    if not added:
        print(f"[DISCOVER] None of the requested files could be resolved: {requested}")
    return "\n\n".join(parts), added
...
JSON schema:
{"pipeline_type":"string","root_cause":"one sentence","confidence":0.0-1.0,"context_sufficient":true,"requested_files":["exact/path", ...],"commit_message":"fix: short description","fixes":[{"file":"exact/path","reason":"what changed","edits":[{"find":"exact text from the file","replace":"corrected text"}]}]}

YOU DO ALL THE REASONING. Nothing upstream has pre-diagnosed this failure for you: decide the root cause yourself, decide whether the files shown below are enough to fix it safely, and decide exactly what to change.

CONTEXT SUFFICIENCY — check this FIRST:
- If the file contents shown below are enough to understand and safely fix the failure, set "context_sufficient": true and proceed to "fixes" as normal.
- If they are NOT enough (e.g. the log points at a file, a workflow, or an entrypoint that isn't shown below), set "context_sufficient": false, leave "fixes" empty, and list the exact repo-relative paths you need in "requested_files" (max 5, real-looking paths only, not guesses). Do not guess a fix without enough information — asking for the right file is better than a wrong edit. You will be given those files and asked again.
...
- Base your root_cause ONLY on the actual error text and the file contents you were given. If a file, a package, or a specific line is named, fix THAT — plus every other issue you can see in the same pass.
...
def main():
    ...
    # ══ STAGE 3: ANALYSE ═════════════════════════════════════════════════════
    # Pure-AI: the model gets the error, the tech/pipeline fingerprint, and the
    # file contents discovered above, and does ALL the reasoning — root cause,
    # whether the context is sufficient, which files to change, and the fix
    # itself. If it says the context isn't enough, we do exactly one more
    # read-and-ask round with the files it requested, then accept its answer.
    print("\n━━━ STAGE 3: ANALYSE ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
    t3 = time.time()
    try:
        ...
        if token and repo:
            open_issue(token, repo, f"AI call failed: {exc}")
        sys.exit(2)

    if not ai_data.get("context_sufficient", True) and ai_data.get("requested_files"):
        requested = ai_data.get("requested_files") or []
        print(f"[ANALYSE] AI says context is insufficient — requested: {requested}")
        extra_context, extra_included = read_extra_files(requested, included)
        if extra_context:
            repo_context += ("\n\n" + extra_context)
            included += extra_included
            print(f"[ANALYSE] Added {len(extra_included)} requested file(s) — asking again.")
            try:
                raw     = call_ai(error_signal, repo_context, pipeline_types, tech_stacks)
                ai_data = parse_ai_response(raw)
            except Exception as exc:
                print(f"[ERROR] AI failed on second pass: {exc}", file=sys.stderr)
                if token and repo:
                    open_issue(token, repo, f"AI call failed on second pass: {exc}")
                sys.exit(2)
        else:
            print("[ANALYSE] None of the requested files could be resolved in the repo — "
                  "proceeding with what the AI already has.")
