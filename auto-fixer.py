...
def _workflow_triggers(content: str):
    """Return the parsed `on:` triggers of a workflow, or None. Handles the
    YAML 1.1 gotcha where a bare `on:` key parses as the boolean True."""
    try:
...
def _is_meta_workflow(content: str) -> bool:
    """A meta/trigger workflow runs *after another workflow* (on: workflow_run)
    — it's the thing that launches this auto-fixer, never a fix target. That's
    a structural signal, not a name match, so it catches the trigger workflow
    regardless of how it invokes the fixer. The string markers stay as a
    fallback for anything that references the fixer directly."""
    on = _workflow_triggers(content)
    keys = set()
    ...
    if "workflow_run" in keys:
        return True
    return any(m in content.lower() for m in META_WORKFLOW_MARKERS)


def _signal_overlap(content: str, signal: str) -> int:
    """How many distinct significant tokens from the failure signal appear in
    this workflow. The CI workflow that actually failed will contain the very
    strings the error mentions (a typo'd path, a bad version) — the unrelated
    trigger workflow won't. This is what picks the RIGHT workflow when a repo
    has several."""
    c = content.lower()
    toks = {t for t in re.findall(r"[\w./-]{4,}", signal.lower())}
    return sum(1 for t in toks if t in c)


def seed_workflow_evidence(allowed_files: set, signal: str = "") -> dict:
    """Seed the investigation with the workflow file that defines the failing
    CI run.
    ...
    Trigger/meta workflows (the ones that launch this fixer) are excluded, and
    the remaining candidates are ranked by how much they overlap the failure
    signal so the ACTUAL failing workflow is the one seeded — not whichever
    ...
    for f in wf_files:
        ...
        if content is None:
            continue
        if _is_meta_workflow(content):
            print(f"[EVIDENCE]   skip meta-workflow {f} "
                  f"(triggered by another run / launches the fixer)")
            continue
        candidates.append((f, content))

    # most relevant to the failure first; stable order for ties
    candidates.sort(key=lambda fc: _signal_overlap(fc[1], signal), reverse=True)

    seeded = {}
    for f, content in candidates[:MAX_SEED_WORKFLOWS]:
        score = _signal_overlap(content, signal)
        if len(content) > MAX_SEED_FILE_CHARS:
            content = content[:MAX_SEED_FILE_CHARS] + "\n...(truncated)"
        seeded[f] = content
        print(f"[EVIDENCE] seeded {f} (signal overlap {score})")
    if not seeded:
...
def main():
    ...
    # failure's fix lives in either app code or the workflow itself. Ranked by
    # overlap with the failure signal so the ACTUAL failing workflow is seeded.
    seed_evidence = seed_workflow_evidence(allowed_files, signal)
